...
class TensorProductRescale(torch.nn.Module):
    ...
    def init_rescale_bias(self) -> None:
        ...
        self.irreps_bias_dims = [
            int(irrep_str.split("x")[0]) for irrep_str in str(self.irreps_bias).split("+")
        ]
        if self.use_bias:
            self.bias = []
...
if __name__ == "__main__":
    ...
    print(f"[TP] before rescaling difference: {torch.max(torch.abs(out_tp - out_tp_rs))}")

    tp_rs.rescale = True
    tp_rs.init_rescale_bias()
    out_tp_rs = tp_rs.forward(inputs_1, inputs_2)
    print(f"[TP] after rescaling difference: {torch.max(torch.abs(out_tp - out_tp_rs))}")
    ...
    print(f"[FCTP] before rescaling difference: {torch.max(torch.abs(out_fctp - out_fctp_rs))}")

    fctp_rs.rescale = True
    fctp_rs.init_rescale_bias()
    out_fctp_rs = fctp_rs.forward(inputs_1, inputs_2)
    print(f"[FCTP] after rescaling difference: {torch.max(torch.abs(out_fctp - out_fctp_rs))}")
