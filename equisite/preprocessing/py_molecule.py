...
from .py_mol_io import load_protein_mol2, load_protein_pdb
...
class PyMolecule:
    ...
    def compute_covalent_bonds(self) -> None:
        ...
        for atomIter in range(len(self.atomPos_[0])):
            ...
            dist = (
                np.linalg.norm(curPos.reshape(1, 3) - self.atomPos_[0], axis=1)
                - curCovRadius
                - covRadii
                - 0.56
            )
    ...
    def load_molecular_file(
        ...
        if pFileType == "pdb":
            # Load the pdb file.
            atomPos, atomTypes, atomNames, _, _, _, transCenter = load_protein_pdb(
                pFilePath, pLoadAnim, pLoadHydrogens=pLoadHydrogens, pLoadGroups=True
            )
        elif pFileType == "mol2":
            # Load mol2 file.
            atomPos, atomTypes, atomNames, _, _, _, transCenter = load_protein_mol2(
                pFilePath, pLoadHydrogens=pLoadHydrogens, pLoadGroups=True
            )
