...
def save_protein_pdb(pFilePath, pProtein):
    ...
    with open(pFilePath, "w") as protFile:
        ...
        for curAtomIter in range(len(pProtein.atomTypes_)):
            ...
            xPosText = f"{curAtomPosition[0]:8.3f}"
            yPosText = f"{curAtomPosition[1]:8.3f}"
            zPosText = f"{curAtomPosition[2]:8.3f}"
            occupancy = "  1.00"
            tempFactor = "  1.00"
            atomType = pProtein.periodicTable_.labels_[pProtein.atomTypes_[curAtomIter]].split("/")[
                0
            ]
            protFile.write(
                ("ATOM  %5d  %s %s %s%4d    %s%s%s%s%s           %s\n")
                % (
                    curAtomIter + 1,
                    curAtomName,
                    resName,
...
def save_molecule_pdb(pFilePath, pMolecule):
    ...
    with open(pFilePath, "w") as protFile:
        for curAtomIter in range(len(pMolecule.atomTypes_)):
            if pMolecule.atomNames_ is not None:
            ...
            tempFactor = "  1.00"
            aminoCounter = 1
            atomType = pMolecule.periodicTable_.labels_[pMolecule.atomTypes_[curAtomIter]].split(
                "/"
            )[0]
            protFile.write(
                ("ATOM  %5d  %s %s %s%4d    %s%s%s%s%s           %s\n")
...
def load_protein_mol2(
    ...
    with open(pFilePath) as datasetFile:
                ...
                if atomSection:
                    ...
                    if curAtomType != "H" or pLoadHydrogens:
                        ...
                        if curResidueName != "HOH" or pLoadH2O:
                            ...
                            if curResidueIndex not in residueIndexDict:
                                curVector = []
                                residueIndexDict[curResidueIndex] = []
                            else:
                                curVector = residueIndexDict[curResidueIndex]

                            if curAtomName not in curVector and (
